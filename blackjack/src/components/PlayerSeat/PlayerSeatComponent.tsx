...
import { BalanceStyled, PlayersWrapper, SeatStyled } from "./Seat.styled";
...
export const PlayerSeatComponent: FC<PlayerProps> = observer(({ id }) => {
  const handleSetNewBet = useCallback(() => {
    if (!gameTable.roundIsStarted) {
      const player = gameTable.seats[id]
        ? gameTable.seats[id][0]
  ...
  return (
    ...
    >
      ...
      <BalanceStyled>
        {gameTable.seats[id] &&
          gameTable.seats[id].filter((player) => !player.parentPlayer)[0]
            .balance}
      </BalanceStyled>
    </SeatStyled>
  );
